...
<?php if(validation_errors()): ?>
  <div class="form_errors">
    <h3>The announcement couldn't be saved because of the following error(s):</h3>
	  <?= validation_errors() ?>
    <?php  $announcement['content'] = $_POST['content'];
...
<?= form_open('announcements/' + $form) ?>
  ...
  <p><label for="start_date">Start date:</label>
	<input type="input" name="start_date" size="10" value="<?php if (isset($announcement)) echo $announcement['start_date']; else echo date("d m Y"); ?>" />
  <span class="note"><tt>dd mm yyyy</tt></span></p>
  
  <p><label for="end_date">End date:</label>
	<input type="input" name="end_date" size="10"<?php if (isset($announcement)) echo ' value="'.$announcement['end_date']; ?>" />
  <span class="note"><tt>dd mm yyyy</tt></span></p>
